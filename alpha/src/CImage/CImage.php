...
class CImage
{
    ...
    const JPEG_QUALITY_DEFAULT = 60;		// Constant for default image quality when not set
    private $quality; 						// Quality level for JPEG images.
    private $useQuality = false; 			//  Is the quality level set from external use (true) or is it default (false)?

    const PNG_COMPRESSION_DEFAULT = -1; 	// Constant for default image quality when not set
    private $compress;						// Compression level for PNG images.
    private $useCompress = false;			// Is the compress level set from external use (true) or is it default (false)?
    private $bgColorDefault = array(		// Default background color to use (rgba).
        'red'   => 0,
        'green' => 0,
        'blue'  => 0,
        'alpha' => null,
    );
    private $bgColor;						// Background color to use, specified as part of options.

    private $saveFolder;					// Where to save the target file.
    private $image;							// The working image object.
    private $imageFolder;					// The root folder of images (only used in constructor to create $pathToImage?).
    private $imageSrc;						// Image filename, may include subdirectory, relative from $imageFolder
    private $pathToImage;					// Actual path to the image, $imageFolder . '/' . $imageSrc
    private $fileType;						// File type for source image, as provided by getimagesize()
    private $extension;						// File extension to use when saving image.
    private $outputFormat = null;			// Output format, supports null (image) or json.
    private $verbose = false;				// Verbose mode to print out a trace and display the created image.
    private $log = array();					// Keep a log/trace on what happens.

    private $palette;						// Handle image as palette image

    private $cacheFileName;					// Target filename, with path, to save resulting image in.
    private $saveAs;						// Set a format to save image as, or null to use original format.
    ...
    private $newWidth;						// New image dimensions, incoming as argument or calculated.
    private $newWidthOrig;  				// Save original value
    private $newHeight;						// New image dimensions, incoming as argument or calculated.
    private $newHeightOrig; 				// Save original value

    private $dpr = 1;						// Change target height & width when different dpr, dpr 2 means double image dimensions.

    const UPSCALE_DEFAULT = true;			// Always upscale images, even if they are smaller than target image.
    private $upscale = self::UPSCALE_DEFAULT;

    public $crop;							// Array with details on how to crop, incoming as argument and calculated.
    ...
    private $fillToFit;							// Resize strategy to fill extra area with background color (true or false).
    private $offset;							// Used with option area to set which parts of the image to use.
    private $fillWidth;							// Calculate target dimension for image when using fill-to-fit resize strategy.
    private $fillHeight;						// Calculate target dimension for image when using fill-to-fit resize strategy.

    private $allowRemote = false;				// Allow remote file download, default is to disallow remote file download.
    private $remotePattern = '#^https?://#';	// Pattern to recognize a remote file.
    private $useCache = true;					// Use the cache if true, set to false to ignore the cached file.
